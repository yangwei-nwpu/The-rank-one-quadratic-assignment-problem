...
int main()
{
	...
	cout << fixed;
	cout << "Average Execution Time: " << total_execution_time  << endl;
	cout << endl;
	cout << endl;
	FILE *fp;
	errno_t err;
...
int minimizeB(int** b, int n)
{
	...
	try {
		...
		long  objectiveValue = cplex.getObjValue();
		clock_t end = clock();
...
void solve(int** a, int** b, int** c, int n, int count)
{
	...
	try {
		...
		//cplex.setParam(IloCplex::Param::MIP::Tolerances::Integrality,1e-12);// IloCplex::Param::MIP::Tolerances::MIPGap 
		...
		obj = ax*bx + cx - alphabeta;
		cout << endl;
		cout << "A(X): " << ax << endl;
		cout << "B(X): " << bx << endl;
		cout << "C(X): " << cx << endl;
		cout << "obj=" << obj << endl;
		long long exactresult = (long long)cplex.getObjValue();
		long long exactresult2 = (long long)exactresult - alphabeta;
		cout << exactresult << " " << exactresult2 << endl;
		//begin of diplaying re<<sult
		...
		fprintf(fp, "milp2_gen_total_time：%f  ", execution_time);
		fprintf(fp, "milp2_gen_solve_time：%f total_time_of_A(X)B(X)'s lower and upper time：%f ", timey, timex);
