...
//char * sol = "C:/Users/wei yang/Desktop/test/test50/QR1-Rgen-L/milp3result/solutionfileQR1-2000-Rgen-L";
int main()
{
...
void matrix_transform(int ** a, int ** b, int ** c, int n)
{
	...
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
		{
			c[i][j] = c[i][j] - alp*b[i][j] - beta1*a[i][j];
			a[i][j] = a[i][j] - ap[i];
			b[i][j] = b[i][j] - bp[i];
			
		}
	alphabeta = (long long)alp*beta1;
}
...
void solve(int** a, int** b, int** c, int n, int count)
{
	...
	try {
		...
		IloArray<IloNumVarArray>  w(env, alpha);
		for (int k = 0; k < alpha; k++)
		{
			w[k] = IloNumVarArray(env, beta);
		}
		char  bufii[16];
		char  bufjj[16];
		...
		//cplex.setParam(IloCplex::Param::Threads, 0);//0 is default automatic ,1 is single thread, n decided by you
		...
		//IloCplex::Param::MIP::Strategy::HeuristicFreq HeurFreq
		//cplex.setParam(IloCplex::Param::Preprocessing::Aggregator, 0);
		//cplex.setParam(IloCplex::Param::Barrier::ConvergeTol, 1e-12);//default : 1e-8
		cplex.setParam(IloCplex::Param::MIP::Display, 3);//输出在第某个节点找到的整数解和时间
		cplex.setParam(IloCplex::Param::MIP::Interval, 1);//default，间隔多少个节点写入一行
		...
		obj = (long long)ax*bx + cx - alphabeta;
		...
		cout << "C(X): " << cx << endl;
		cout << "obj=" << obj << endl;
		long long exactresult = (long long)(IloRound(cplex.getObjValue())-alphabeta);
		...
		fprintf(fp, "milp3_gen_total_time：%f  ", execution_time);
		fprintf(fp, "milp3_gen_solve_time：%f total_time_of_A(X)B(X)'s lower and upper：%f \n", timey, timex);
